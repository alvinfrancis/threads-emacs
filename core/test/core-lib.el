...
;; `threads-unquote'
(def-test! unquote
  (should (equal (threads-unquote '(quote (a b c))) '(a b c)))
  ;; nested
  (should (equal (threads-unquote '(quote (quote (a b c)))) '(a b c)))
  ;; sub-quote
  (should (equal (threads-unquote '(quote (a (quote b) c))) '(a (quote b) c)))
  ;; function
  (should (equal (threads-unquote '(function a)) 'a)))
